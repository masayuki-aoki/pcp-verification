...
\(\mathrm{PCp}(X)\) with **safe rounding** and **exact** local factors, plus an exact small-\(X\) enumerator.
The outputs are hashed (SHA-256) and recorded to enable fully reproducible verification.
...
* Certifies **every even X in [80,000, 90,000]** with (\mathrm{PCp}(X)>0).
...
* Same output structure under `outputs/optimized/`.
...
* Computes (\mathrm{PCp}(X)) exactly via FFT convolution for **X ≤ 80,000**.
* Expected zero-set:
...
* BV penalty: computed with `Decimal` using **ROUND_CEILING** (upwards) and aggregated as
  ( \mathrm{BV}*{\text{tot}} = \bigl(1 + V(z)f(3)\bigr) \cdot \mathrm{BV}*{\text{base}} ).
* Local factor (V(z)): exact rational product over primes **ℓ ≥ 3** up to (z=\lfloor \sqrt{X}\rfloor+1).
...
Each output has a per-file `.sha256` next to it, and the repo root contains a consolidated
**`SHA256SUMS`** for all `outputs/**/*.txt` and `outputs/**/*.json`:
